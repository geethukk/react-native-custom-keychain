...
async () => {
  ...
  try {
    ...
    const credentials = await Keychain.getItem();
    if (credentials) {
      console.log('Credentials successfully loaded for user ' + credentials.key);
    } else {
...
Both `setItem` and `setInternetCredentials` are limited to strings only, so if you need to store objects etc, please use `JSON.stringify`/`JSON.parse` when you store/access it.

### `setItem(key, value, [{ accessControl, accessible, accessGroup, service, securityLevel }])`

Will store the key/value combination in the secure storage. Resolves to `true` or rejects in case of an error.

### `getItem([{ authenticationPrompt, service }])`
...
### `setInternetCredentials(server, key, value, [{ accessControl, accessible, accessGroup, securityLevel }])`

Will store the server/key/value combination in the secure storage.

### `hasInternetCredentials(server, [{ authenticationPrompt }])`

Will check if the key/value combination for server is available in the secure storage. Resolves to `true` if an entry exists or `false` if it doesn't.

### `getInternetCredentials(server, [{ authenticationPrompt }])`

Will retrieve the server/key/value combination from the secure storage. Resolves to `{ key, value }` if an entry exists or `false` if it doesn't. It will reject only if an unexpected error is encountered like lacking entitlements or permission.

### `resetInternetCredentials(server)`

Will remove the server/key/value combination from the secure storage.

### `requestSharedWebCredentials()` (iOS only)

Asks the user for a shared web credential. Requires additional setup both in the app and server side, see [Apple documentation](https://developer.apple.com/documentation/security/shared_web_credentials). Resolves to `{ server, key, value }` if approved and `false` if denied and throws an error if not supported on platform or there's no shared credentials.

### `setSharedWebCredentials(server, key, value)` (iOS only)
...
|**`authenticationPrompt`**|iOS only|What to prompt the user when unlocking the keychain with biometry or device value. |`Authenticate to retrieve secret`|
...
|**`service`**|All|Reverse domain name qualifier for the service associated with value. |*App bundle ID*|
...
|**`APPLICATION_PASSWORD`**|Constraint to use an application-provided value for data encryption key generation.|
...
The `setInternetCredentials(server, key, value)` call will be resolved as call to `setItem(key, value, server)`. Use the `server` argument to distinguish between multiple entries.

### iOS

If you need Keychain Sharing in your iOS extension, make sure you use the same App Group and Keychain Sharing group names in your Main App and your Share Extension. To then share the keychain between the Main App and Share Extension, use the `accessGroup` and `service` option on `setItem` and `getItem`, like so: `getItem({ accessGroup: 'group.appname', service: 'com.example.appname' })`
